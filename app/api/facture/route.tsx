  ...
  try {
    const factures = await prisma.facture.findMany({
      include: {
        ...
        utilisateur: true, // Inclure les utilisateurs associés
      },
    });
...
export async function POST(request: Request) {
  ...
  try {
    const { date_creation, montant_total, id_reservation, id_utilisateur } =
      await request.json();
    ...
    return NextResponse.json(
      { message: "facture créé avec succès", factures },
      { status: 201 }
    );
  } catch (error) {
